...
/* Poll Implementation instead of select */ 
#include <sys/poll.h> 
/* */
...
int make_tcp_frame(uint8_t * buffer, size_t size, struct send_tcp_frame * tcp_frame)
{
    switch(tcp_frame->type)
    {
        ...
        {
            if(size < 10+tcp_frame->data_size)
                return -1;
            buffer[0] = TCP_FRAME_SEND_RECEIVE;
            memcpy(buffer+1, tcp_frame->address, 8);
            buffer[9] = tcp_frame->rssi;
            memcpy(buffer+10, tcp_frame->data, tcp_frame->data_size);
            return 10+tcp_frame->data_size;
        }break;
    }
    return -1;
}
...
void networkListen(int sd, int fd)
{
    ...
    while(ret > 0)
    {
        ...
        /* Poll */
        fdset.fd = sd;
        fdset.events = POLLIN | POLLERR | POLLHUP;
        ERR(poll(&fdset, 1, -1), "poll");
...
void setAPI(int fd)
{
    ...
    ERR(size=read(fd, buf+1, 1), "read");
    ERR(size=read(fd, buf+2, 1), "read");
    buf[3] = '\0';
    if(strncmp(buf, "OK", 2)!=0){ fprintf(stderr, "Module Not Responding (%s)\n", buf); close(fd); exit(1);}
    fprintf(stderr, "XBee Command Mode : %s\n", buf);
    /*ERR(write(fd, "ATMY FFFF\r", 10), "write");
    ERR(size=read(fd, buf, 1024), "read");
    if(strncmp(buf, "OK", 2)!=0){ fprintf(stderr, "Module Not Responding\n"); exit(1);}
...
int main(int argc, char **argv)
{
    ...
    if(argc > 1)
        strcpy(comm_port, argv[1]);
    else
        strcpy(comm_port, COMM_PORT);
